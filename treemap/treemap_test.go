...
func TestNewWith(t *testing.T) {
	m := NewWith[User, any](UserComparator)
	if m.Size() != 0 {
		t.Error()
	}
	if !m.Empty() {
...
func TestEnumerate(t *testing.T) {
	...
	for idx, entry := range m.Enumerate() {
		if entry.Key != idx || entry.Value != idx {
			t.Error()
		}
	}
}
...
func benchmarkPut(b *testing.B, m *TreeMap[int, struct{}], size int) {
	for i := 0; i < b.N; i++ {
		for n := 0; n < size; n++ {
			m.Put(n, struct{}{})
		}
...
func benchmarkRemove(b *testing.B, m *TreeMap[int, struct{}], size int) {
	for i := 0; i < b.N; i++ {
		for n := 0; n < size; n++ {
			m.Remove(n)
		}
...
func benchmarkGet(b *testing.B, m *TreeMap[int, struct{}], size int) {
	for i := 0; i < b.N; i++ {
		for n := 0; n < size; n++ {
			m.Get(n)
		}
